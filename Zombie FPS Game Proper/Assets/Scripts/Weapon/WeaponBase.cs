...
public class WeaponBase : MonoBehaviour {
	...
	void Update() {
		if(fireMode == FireMode.SEMI && Input.GetKeyDown(KeyCode.Mouse0) && !isReloading) {
			Fire();
		}
		else if(fireMode == FireMode.AUTO && Input.GetKeyDown(KeyCode.Mouse0) && !isReloading) {
			Fire();
		}
	...
	Vector3 CalculateSpread(float inaccuracy, Transform trans) {
		if(Input.GetButtonDown("Fire2")) inaccuracy /= 2;
